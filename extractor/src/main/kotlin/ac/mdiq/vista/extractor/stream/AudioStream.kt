...
* AudioStream.kt is part of Vista Guide.
*
 ...
 * @param content        the content or the URL of the stream, depending on whether isUrl istrue
 ...
 * @param averageBitrate the average bitrate of the stream (which can be unknown, see [.UNKNOWN_BITRATE])
...
class AudioStream private constructor(
    ...
    override fun equalStats(other: Stream?): Boolean {
        return (super.equalStats(other) && other is AudioStream && averageBitrate == other.averageBitrate) && audioTrackId == other.audioTrackId && audioTrackType == other.audioTrackType && audioLocale == other.audioLocale
    }
    ...
    class Builder {
         ...
         * The identifier and the content (and so the `isUrl` boolean) properties must have been set.
        ...
        fun build(): AudioStream {
            checkNotNull(id) { ("The identifier of the audio stream has been not set or is null. If you are not able to get an identifier, use the static constant ID_UNKNOWN of the Stream class.") }
            checkNotNull(content) { ("The content of the audio stream has been not set or is null. Please specify a non-null one with setContent.") }
            checkNotNull(deliveryMethod) { ("The delivery method of the audio stream has been set as null, which is not allowed. Pass a valid one instead with setDeliveryMethod.") }

            return AudioStream(id!!, content!!, isUrl, mediaFormat, deliveryMethod!!, averageBitrate, manifestUrl, audioTrackId, audioTrackName, audioLocale, audioTrackType, itagItem)
        }
    }
