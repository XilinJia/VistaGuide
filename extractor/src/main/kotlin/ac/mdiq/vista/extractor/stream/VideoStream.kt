...
* VideoStream.kt is part of Vista Guide.
*
 ...
 * @param id             the identifier which uniquely identifies the stream, e.g. for YouTube this would be the itag
 * @param content        the content or the URL of the stream, depending on whether isUrl is true
...
class VideoStream private constructor(
    ...
    override fun equalStats(other: Stream?): Boolean {
        return ((super.equalStats(other) && other is VideoStream) && resolution == other.resolution) && isVideoOnly == other.isVideoOnly
    }
    ...
    class Builder {
         ...
         * [ItagItem]s are YouTube specific objects, so they are only known for this service and can be null.
         * The default value is `null`.
        ...
        fun build(): VideoStream {
            checkNotNull(id) { ("The identifier of the video stream has been not set or is null. If you are not able to get an identifier, use the static constant ID_UNKNOWN of the Stream class.") }

            checkNotNull(content) { ("The content of the video stream has been not set or is null. Please specify a non-null one with setContent.") }

            checkNotNull(deliveryMethod) { ("The delivery method of the video stream has been set as null, which is not allowed. Pass a valid one instead with setDeliveryMethod.") }

            checkNotNull(isVideoOnly) { ("The video stream has been not set as a video-only stream or as a video stream with embedded audio. Please specify this information with setIsVideoOnly.") }

            checkNotNull(resolution) { ("The resolution of the video stream has been not set. Please specify it with setResolution (use an empty string if you are not able to get it).") }

            return VideoStream(id!!, content!!, isUrl, mediaFormat, deliveryMethod!!, resolution!!, isVideoOnly!!, manifestUrl, itagItem)
        }
    }
