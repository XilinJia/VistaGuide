...
import ac.mdiq.vista.extractor.utils.JavaScript.run
import java.util.*
...
object YoutubeJavaScriptPlayerManager {
    ...
    fun getSignatureTimestamp(videoId: String): Int {
        ...
        if (sigTimestampExtractionEx != null) throw sigTimestampExtractionEx!!

        extractJavaScriptCodeIfNeeded(videoId)

        try {
            cachedSignatureTimestamp = YoutubeSignatureUtils.getSignatureTimestamp(cachedJavaScriptPlayerCode!!).toInt()
    ...
    fun deobfuscateSignature(videoId: String, obfuscatedSignature: String): String {
        ...
        // changed
        // Throw again the corresponding stored exception in this case to improve performance
        if (sigDeobFuncExtractionEx != null) throw sigDeobFuncExtractionEx!!
        ...
        if (cachedSignatureDeobfuscationFunction == null) {
            try {
                ...
                sigDeobFuncExtractionEx = ParsingException("Could not get signature parameter deobfuscation JavaScript function", e)
                throw e
            }
        }
        ...
        try {
            ...
            return Objects.requireNonNullElse(
                run(cachedSignatureDeobfuscationFunction, YoutubeSignatureUtils.DEOBFUSCATION_FUNCTION_NAME, obfuscatedSignature), "")
        } catch (e: Exception) {
            // This shouldn't happen as the function validity is checked when it is extracted
            throw ParsingException("Could not run signature parameter deobfuscation JavaScript function", e)
        }
    }
     ...
     * improve performance with multiple calls of this method having the same obfuscated throttling parameter.
     *
     ...
     */
    @Throws(ParsingException::class)
    fun getUrlWithThrottlingParameterDeobfuscated(videoId: String, streamingUrl: String): String {
        val obfuscatedThrottlingParameter = YoutubeThrottlingParameterUtils.getThrottlingParameterFromStreamingUrl(streamingUrl) ?: return streamingUrl
        ...
        if (cacheResult != null) return streamingUrl.replace(obfuscatedThrottlingParameter, cacheResult)
        ...
        // been not changed
        // Throw again the corresponding stored exception in this case to improve performance
        if (throttlingDeobfFuncExtractionEx != null) throw throttlingDeobfFuncExtractionEx!!
        ...
        if (cachedThrottlingDeobfuscationFunction == null) {
            try {
                ...
                throttlingDeobfFuncExtractionEx = ParsingException("Could not get throttling parameter deobfuscation JavaScript function", e)
                throw e
            }
        }

        try {
            val deobfuscatedThrottlingParameter = run(cachedThrottlingDeobfuscationFunction, cachedThrottlingDeobfuscationFunctionName, obfuscatedThrottlingParameter)
            CACHED_THROTTLING_PARAMETERS[obfuscatedThrottlingParameter] = deobfuscatedThrottlingParameter
            return streamingUrl.replace(obfuscatedThrottlingParameter, deobfuscatedThrottlingParameter)
        } catch (e: Exception) {
            // This shouldn't happen as the function validity is checked when it is extracted
            throw ParsingException("Could not run throttling parameter deobfuscation JavaScript function", e)
        }
    }
    ...
    private fun extractJavaScriptCodeIfNeeded(videoId: String) {
        if (cachedJavaScriptPlayerCode == null) cachedJavaScriptPlayerCode = extractJavaScriptPlayerCode(videoId)
    }
}
